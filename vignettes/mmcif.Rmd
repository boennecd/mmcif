---
bibliography: mmcif-comp-details.bib
---
...
[![CRAN RStudio mirror downloads](http://cranlogs.r-pkg.org/badges/mmcif)](https://CRAN.R-project.org/package=mmcif)
...
competing risk. For interested readers, a vignette on computational details can 
be found by calling `vignette("mmcif-comp-details", "mmcif")`.
...
the trajectory must be constrained to be monotonically decreasing in $t$. The 
covariates for the trajectory in this package are defined as 

$$\vec x_{ij}(t) = (\vec h(t)^\top, \vec z_{ij}^\top)^\top$$

for a spline basis $\vec h$ and known covariates $\vec z_{ij}$ which are also 
used in the risk part of the model.
...
functions when the random effects are zero and the covariates are zero, 
$a_{ij} = b_{ij} = 0$.
...
We may reduce the estimation time by using a different number of quadrature 
nodes starting with fewer nodes successively updating the fits as shown below.

```{r successive_fits, cache = 1}
...
The `mmcif_pd_univariate` function is provided to compute the marginal CIFs, 
the derivative of the marginal CIFs, and the marginal survival probability.

```{r pre_show_mmcif_pd_univariate, echo = FALSE}
comp_obj <- mmcif_data(
  ~ a + b, dat, cause = cause, time = time, cluster_id = cluster_id,
  max_time = delta, spline_df = 4L)
```

```{r show_mmcif_pd_univariate}
...
There is a bivariate version as well. This gives the marginal bivaraite CIFs, 
marginal survival probabilities, the marginal density, and mixtures thereof 
as illustrated below.

```{r show_mmcif_pd_bivariate}
...
Finally, there is also function to compute the marginal density, CIF, survival 
probability or hazard conditional on the outcome of another individual.

```{r show_mmcif_pd_cond}
...
```{r delayed_sandwich, ref.label = "sandwich", cache = 1}
```
...
```{r delayed_compare_estimated_incidence_funcs, ref.label="compare_estimated_incidence_funcs", cache = 1, fig.height=knitr::opts_chunk$get("fig.width") * .5}
```
...
where there are $L$ strata each having their own spline basis 
$h_{l}(t)$ and $l_{ij}$ is the strata that observation $j$ in cluster $i$ is 
in. This is supported in the package using the `strata` argument of 
`mmcif_data`. We illustrate this by extending the previous example.
First, we assign new model parameters and plot the cumulative incidence 
functions as before but for each strata. 

```{r strata_assign_model_parameters, fig.height=knitr::opts_chunk$get("fig.width") * .5}
# assign model parameters
n_causes <- 2L
delta <- 2
...
```{r strata_delayed_assign_sim_dat}
library(mvtnorm)
...
```{r strata_delayed_sample_data, cache = 1}
# sample a data set
set.seed(14712915)
n_clusters <- 1000L
max_cluster_size <- 5L
...
```{r strata_sandwich, ref.label = "sandwich", cache = 1}
```
...
```{r strata_compare_estimated_incidence_funcs, fig.height=knitr::opts_chunk$get("fig.width") * .5, cache = 1}
local({
  # get the estimates
  coef_risk_est <- fit$par[comp_obj$indices$coef_risk] |> 
    matrix(ncol = n_causes)
...
```{r pre_three_assign_model_parameters, echo = FALSE}
rm(list = ls())
```

In this section, we show an example with $K = 3$ causes. First, we assign 
the parameters and plot the  cumulative incidence functions when the 
random effects are zero and the covariates are zero.

```{r three_assign_model_parameters}
# assign model parameters
n_causes <- 3L
delta <- 2
...
Then we assign a simulation function like before with delayed entry 
but with the additional cause.

```{r three_assign_sim_dat}
library(mvtnorm)
...
```{r three_check_comp_time, cache = 1}
NCOL(comp_obj$pair_indices) # the number of pairs in the composite likelihood
length(comp_obj$singletons) # the number of clusters with one observation
...
```{r three_sandwich, cache = 1}
system.time(sandwich_est <- mmcif_sandwich(comp_obj, fit$par, n_threads = 4L))
```

#### Estimates

We show the estimated and true the conditional cumulative incidence functions 
(the dashed curves are the estimates) like before.

```{r three_compare_estimated_incidence_funcs, cache = 1}
local({
  # get the estimates
  coef_risk_est <- fit$par[comp_obj$indices$coef_risk] |> 
    matrix(ncol = n_causes)
